...
void SEQ(char* b1, char* b1_2, char* b2, char* b2_2, char* tm, char* tm2, data_t* data)
{
	...
	long totalBytes = (long)data->blockSize * (long)NUM_IT * NUM_IT * 6;
	...
	fin = clock() - start;
	double divis = CLOCKS_PER_SEC * NUM_IT * NUM_IT;
...
void RAND(char* b1, char* b1_2, char* b2, char* b2_2, char* tm, char* tm2, data_t* data){
	...
	long totalBytes = (long)data->blockSize * (long)NUM_IT * mult * 6;
	transfTime = (t_total/ CLOCKS_PER_SEC) - emptyTime ;
...
void memory() {
	pthread_t thread; 
	struct data_t data; 
	data.blockSize = 1024*1024;
	
	pthread_create(&thread, NULL, rwOp, (void *)&data);
